...
- **Parts**: 個別部品画像（小さな画像）
- **Tasks**: ユーザーからの部品請求
  - 住所フィールド: `prefecture`, `city`, `town`, `address_detail`, `building_name`
  - フロータイプ: `assembly_number` / `photo_parts`
...
│       ├── src/
│       │   ├── app/         # App Router ページ
│       │   │   └── api/     # APIルート (PDF生成, メール送信)
│       │   ├── components/  # UIコンポーネント
│       │   │   ├── steps/   # 申請フローの各ステップ
│       │   │   └── ui/      # 共通UIコンポーネント
│       │   ├── lib/         # ユーティリティ (Supabase, ZipCloud API)
│       │   └── types/       # 型定義
│       └── package.json
│
├── supabase/
...
1. **購入情報入力**: 製品選択、購入場所・日時、保証コードの入力
2. **パーツ選択**: 2つのフローから選択
   - **組立番号フロー**: 組立画像から必要な部品を選択（座標ベースのオーバーレイ表示）
   - **パーツ写真フロー**: 不足部品の写真をアップロード（フリーハンド描画で印付け可能）
3. **送付先情報入力**:
   - 郵便番号から住所自動入力（ZipCloud API）
   - 住所5フィールド分割（都道府県/市区町村/町域/番地/建物名）
   - メールアドレスバリデーション（RFC 5322準拠）
4. **確認・申請**: 入力内容確認、PDF生成、確認メール送信
...
- WebP形式に変換
- 最大サイズにリサイズ
   - Admin Tool: 2000px　取説キャプチャ用
   - User App: 1000px　ユーザー写真アップ用
