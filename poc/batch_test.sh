...
mkdir -p output/assembly_numbers_v2/batch_test

for f in input/AssemblyImage/sample/*.png; do
    name=$(basename "$f" .png)
    echo "=== $name ==="
    python extract_assembly_numbers_v2.py "$f" --debug -o "output/assembly_numbers_v2/batch_test/$name" 2>&1 | grep -E "(Valid:|DONE)"
done
